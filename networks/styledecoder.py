...
class ToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        ...
        self.conv = ConvLayer(in_channel, 3*32, 1)
...
class Motion_Field(nn.Module):
    def __init__(self, style_dim, motion_dim):
        super(Motion_Field, self).__init__()
    ...
    def forward(self, z_s2c, z_d2c):

        # generating moving directions
        alpha = self.fc(z_d2c)      
        directions_c2d = self.direction(alpha)  # target

        z_s2d = z_s2c + directions_c2d
...
class Plane_dictionary(nn.Module):
    def __init__(self, plane_channels, plane_size, motion_num):
        super(Plane_dictionary, self).__init__()

        self.plane_channels = plane_channels
        self.weight = nn.Parameter(torch.randn(plane_size, 
                                            plane_size, 
                                            plane_channels, 
                                            motion_num))
    ...
    def forward(self, input):
        # 每行进行分解
        # 增加一维,并扩展到同样的通道数
        input = input.unsqueeze(2).repeat(1,1,self.plane_channels,1,1) # (bs, 20, 32, 256, 256)
        weight = self.weight + 1e-8
        ...
        #         Q, _ = torch.linalg.qr(weight[i][j])  # get eignvector, orthogonal [n1, n2, n3, n4]
        #         Q_list_row.append(Q)
        #     Q_ = torch.stack(Q_list_row,dim=0)
        #     Q_column.append(Q_)
        # Q_all = torch.stack(Q_column,dim=0)
        # print(Q_all.shape)
        
        Q_all, _ = torch.linalg.qr(weight)
        
        ortho_planes = Q_all.permute(3,2,1,0).unsqueeze(0).repeat(input.shape[0],1,1,1,1) # (bs,20,32,256,256)
        out = torch.mul(input, ortho_planes) # 对应元素相乘
        out = torch.sum(out, dim=1) # 20个plane线性求和 (bs,32,64,64)
...
class Plane_motion_field(nn.Module):
    def __init__(self, style_dim, plane_channels, plane_size, motion_num):
        super(Plane_motion_field, self).__init__()

        self.dict = Plane_dictionary(plane_channels, plane_size, motion_num)
        ...
        # fc.append(EqualLinear(style_dim, 3*motion_num*plane_size*plane_size))
        # fc.append(EqualLinear(style_dim, style_dim)) # 每个维度一个
        # self.fc = nn.Sequential(*fc)
        self.decoder = SynthesisNetwork(w_dim=style_dim, img_resolution=plane_size, img_channels=motion_num)

        self.fusion = TransformerDecoderLayer(d_model=512, nhead=4, dim_feedforward=512,normalize_type='layernorm')

    def forward(self, z_source, z_target):

        z_animation = self.fusion(z_source,z_target) # [bs,512]

        alpha = self.decoder(z_animation.unsqueeze(1).repeat(1,14,1)) # [bs,20,64,64]
        plane = self.dict(alpha) # [bs,96,256,256] 

        return plane, z_animation