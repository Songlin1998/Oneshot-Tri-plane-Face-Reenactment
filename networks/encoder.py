...
class Encoder_exp(nn.Module):
    def __init__(self, size, dim=512):
        super(Encoder_exp, self).__init__()
        self.net_app = EncoderApp(size, dim)

    def forward(self, input_source):
        lantent_code, _ = self.net_app(input_source)
        return lantent_code

class Encoder_tri(nn.Module):
    def __init__(self, size, dim=512):
        super(Encoder_tri, self).__init__()
        ...
        self.convs = nn.ModuleList()
        self.convs.append(ConvLayer(3*32, channels[size], 1))
...
class Fusion(nn.Module):
    ...
    def forward(self, id_code, exp_code):
        ...
        code = self.fc(torch.cat([id_code,exp_code],dim=1))
        return code