...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img_256, img_128, c, update_emas=False, **block_kwargs):
        
        image_raw = filtered_resizing(img_128, size=256, f=self.resample_filter)
        img = torch.cat([img_256, image_raw], 1)
...
@persistence.persistent_class
class DualDiscriminator_r1(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
    ...
    def forward(self, img_256, img_128, c, r1=False, update_emas=False, **block_kwargs):
        ...
        if r1==False:
            ...
            # r1 regulization: 对判别器中大梯度的惩罚
            r1_gamma = 10
            with conv2d_gradfix.no_weight_gradients():
                # print(x.requires_grad)
                # print(img_256.requires_grad)
                # print(img_128.requires_grad)
                r1_grads = torch.autograd.grad(outputs=[x.sum()], inputs=[img_256, img_128], create_graph=True, only_inputs=True)
                r1_grads_image = r1_grads[0]
                # print(r1_grads_image)
                r1_grads_image_raw = r1_grads[1]
                # print(r1_grads_image_raw)
            r1_penalty = (r1_grads_image.square().sum([1,2,3]) + r1_grads_image_raw.square().sum([1,2,3]))*(r1_gamma / 2)
